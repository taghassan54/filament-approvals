        ...
        <p class="px-3 text-xs">
            <small>
                {{ $getRecord()->isApprovalCompleted() ?
                    __('filament-approvals::approvals.approval_complete') :
                    __('filament-approvals::approvals.approval_in_process') }}
            </small>
        </p>
    @else
        <span class="px-3 py-1 bg-gray-200 text-gray-800 rounded-full text-xs">
            {{ __('filament-approvals::approvals.approval_status_does_not_exist') }}
        </span>
    @endif
</div>
